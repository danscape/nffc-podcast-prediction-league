...
export default async function HomePage() {
  ...
  const inFormPlayer: InFormPlayer = inFormPlayerRow
    ? {
        ...
        points: Number(
          (inFormPlayerRow as InFormPlayerViewRow & {
  ...
  const inFormTeam: InFormTeam = inFormTeamRow
    ? {
        ...
        points: Number(
          (inFormTeamRow as InFormTeamViewRow & {
            points?: number | string | null;
            total_points?: number | string | null;
            form_points?: number | string | null;
            team_points?: number | string | null;
          }).points ??
            (inFormTeamRow as InFormTeamViewRow & {
              total_points?: number | string | null;
            }).total_points ??
      ...
      <div className="grid gap-px bg-[#242424]">
        {confidenceRows.length ? (
          confidenceRows.map((row) => {
            const rowTone =
              Number(row.forest_win_percent ?? 0) > 50
                ? "text-[var(--stat-green,#22e55e)]"
                : Number(row.forest_loss_percent ?? 0) > 50
                  ? "text-[var(--stat-wrong,#ff3030)]"
                  : "text-[var(--stat-yellow,#ffe44d)]";
            ...
            return (
              <div
                ...
                className={`grid grid-cols-[46px_minmax(0,1fr)_112px] items-center bg-[var(--nffc-black,#000000)] px-1 py-1 ${rowTone}`}
              >
                ...
                <div className="truncate text-sm font-black uppercase">
                  {row.opponent_short} <span>{row.venue}</span>
                </div>

                <div className="text-right text-[0.62rem] font-black uppercase">
