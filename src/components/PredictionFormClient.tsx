...
type PlayerWeeklyRankHistoryRow = {
  gameweek: number;
  overall_rank_after_gw: number | null;
};
...
const confirmationEmailDelayMs = 60 * 1000;
...
export default function PredictionFormClient({
  ...
  const [weeklyRankRows, setWeeklyRankRows] = useState<PlayerWeeklyRankHistoryRow[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadWeeklyRanks() {
      const playerId = initialData.player.id;
      ...
      const { data } = await supabase
        ...
        .select("gameweek, overall_rank_after_gw")
        .eq("player_id", playerId);

      if (!cancelled) {
        setWeeklyRankRows((data ?? []) as PlayerWeeklyRankHistoryRow[]);
      }
    }
        ...
        return {
          rows: [
            ...state.rows,
            {
              ...
              overall_rank_after_gw: rankMap.get(Number(score.gameweek)) ?? null,
            },
          ],
          runningTotal: nextRunningTotal,
        };
      ...
      <main className="box-border min-h-screen w-full max-w-none overflow-x-hidden bg-[var(--nffc-black,#000000)] px-0 pt-0 pb-3 text-[var(--nffc-white,#f5f5f5)] md:hidden">
        <section className="box-border w-full max-w-none px-0 pt-0 pb-4">
          ...
          <section className="mb-2 mt-2 min-w-0 px-0">
            ...
            <MobileSeasonPointsTally projection={projection} />
          </section>
...
function MobileTallyCell({
  ...
  return (
    <div className="bg-[var(--nffc-black,#000000)] px-1 py-1.5">
      ...
      <div
        className={`mt-0.5 text-[0.95rem] font-black uppercase leading-none ${
          tone === "green"
            ? "text-[var(--stat-green,#22e55e)]"
            : "text-white"
        }`}
  ...
  return (
    <article className="border-b border-[#242424] px-px py-1 last:border-b-0">
      <button
        type="button"
        onClick={onToggle}
        className="grid w-full grid-cols-[44px_minmax(0,1fr)_34px_52px_62px_22px] items-center gap-1 text-left"
      ...
      >
        ...
        <div className="text-right text-sm font-black uppercase tracking-[0.04em] text-[var(--stat-cyan,#59efff)]">
          {score.overall_rank_after_gw ? `#${score.overall_rank_after_gw}` : "—"}
        </div>
