...
export default function AdminSocialResultsPage() {
  ...
  return (
    <main className="min-h-screen bg-[var(--nffc-black,#000000)] px-2 py-2 font-mono text-[var(--nffc-white,#f5f5f5)] sm:px-3 lg:px-4">
      <section className="w-full bg-[var(--nffc-black,#000000)] p-2">
        <Masthead onRefresh={loadResultsData} />

        {message && (
          <div className="mt-2 border border-[var(--stat-wrong,#ff3030)] bg-[var(--nffc-black,#000000)] px-4 py-3 text-sm font-black text-[var(--stat-wrong,#ff3030)]">
            {message}
          </div>
        )}

        {loading ? (
          <div className="mt-2 border border-[var(--nffc-white,#f5f5f5)] bg-[var(--nffc-panel,#070707)] p-8 text-lg font-black uppercase tracking-[0.18em] text-[var(--stat-yellow,#ffe44d)]">
          ...
          <>
            ...
            <section className="mt-2 grid gap-2 lg:grid-cols-[1fr_420px]">
              <div className="border border-[var(--nffc-white,#f5f5f5)] bg-[var(--nffc-panel,#070707)] p-3">
                <div className="bg-[var(--nffc-red,#e50914)] px-3 py-2 text-sm font-black uppercase tracking-[0.18em] text-white">
                  Matrix purpose
                </div>
                <p className="mt-3 max-w-5xl text-sm font-bold leading-6 text-[var(--nffc-muted,#a7a7a7)]">
                  Simple weekly scoring view: each team row shows who got the Forest result right,
                  who missed, and which bonus chips were applied. The social graphic should be
                  readable at a glance: strong player names, visible weekly totals, and quick team summaries.
                </p>
              </div>
              ...
              <div className="border border-[var(--nffc-white,#f5f5f5)] bg-[var(--nffc-panel,#070707)] p-3">
                ...
                <pre className="mt-2 min-h-[190px] whitespace-pre-wrap border border-[var(--nffc-white,#f5f5f5)] bg-[var(--nffc-black,#000000)] p-2 text-sm font-bold leading-6">
                  {weeklyPost}
                </pre>
                <button
                  type="button"
                  onClick={copyWeeklyPost}
                  className="mt-2 w-full border border-[var(--stat-green,#22e55e)] bg-[var(--nffc-black,#000000)] px-4 py-3 text-sm font-black uppercase tracking-[0.18em] text-[var(--stat-green,#22e55e)] hover:bg-[var(--stat-green,#22e55e)] hover:text-black"
                >
...
function Masthead({ onRefresh }: { onRefresh: () => void }) {
  return (
    <header className="grid gap-2 bg-[var(--nffc-black,#000000)] p-2 lg:grid-cols-[380px_1fr_auto] lg:items-center">
      <div className="flex items-center justify-center bg-[var(--nffc-black,#000000)] px-3 py-2">
        <Image
      ...
      <nav className="flex flex-wrap gap-2 lg:justify-end">
        <button
          type="button"
          onClick={onRefresh}
          className="border border-[var(--stat-green,#22e55e)] bg-[var(--nffc-black,#000000)] px-5 py-4 text-base font-black uppercase tracking-[0.16em] text-[var(--stat-green,#22e55e)] hover:bg-[var(--stat-green,#22e55e)] hover:text-black"
        >
          Refresh
        </button>
        <Link
          href="/admin/social"
          className="border border-[var(--nffc-red,#e50914)] bg-[var(--nffc-black,#000000)] px-5 py-4 text-base font-black uppercase tracking-[0.16em] text-[var(--nffc-red,#e50914)] hover:border-white hover:text-white"
        >
          Social hub
        </Link>
        <Link
          href="/admin"
          className="border border-[var(--nffc-muted,#a7a7a7)] bg-[var(--nffc-black,#000000)] px-5 py-4 text-base font-black uppercase tracking-[0.16em] text-[var(--nffc-muted,#a7a7a7)] hover:border-white hover:text-white"
        >
          Admin
        </Link>
      </nav>
...
function SocialResultsGraphic({
  ...
  return (
    ...
    >
      <div className="flex items-center gap-5 bg-[var(--nffc-red,#e50914)] px-4 py-3 text-2xl font-black uppercase tracking-[0.16em] text-white md:text-4xl">
        <Image
          src="/brand/nffc-podcast-prediction-league-banner.png"
          alt="NFFC Podcast Prediction League"
          width={640}
        ...
        />
        <span>{"// Weekly Results Terminal"}</span>
      </div>
      ...
      <div className="grid gap-px bg-[var(--nffc-white,#f5f5f5)]">
        ...
        <section className="grid gap-px bg-[var(--nffc-white,#f5f5f5)]">
          {teamGroups.map((group, index) => (
            <TeamResultRow key={group.key} group={group} rank={index + 1} summary={summary} />
          ))}
        </section>
      </div>
    </section>
...
function CompactResultsHeader({
  ...
  return (
    <section className="grid gap-0 bg-[var(--nffc-black,#000000)] xl:grid-cols-[minmax(440px,1fr)_360px_360px_340px]">
      <div className="bg-[var(--nffc-black,#000000)] p-2">
        <div className="text-xs font-black uppercase tracking-[0.22em] text-[var(--stat-yellow,#ffe44d)]">
          {`${getGameweekLabel(summary)} // Final scores locked`}
        </div>
        <h2 className="mt-1 text-3xl font-black uppercase leading-tight text-white md:text-5xl">
          {getScoreText(summary)}
        </h2>
      </div>

      <div className="grid grid-cols-3 gap-0 bg-[var(--nffc-black,#000000)]">
        {(["W", "D", "L"] as PredictionValue[]).map((value) => (
          <CompactPredictionPill
            key={value}
            value={value}
      ...
      <CompactSummaryBox label="Top scorers" value={topScorerText} tone="green" />
...
function CompactPredictionPill({
  ...
  return (
    ...
    >
      <div
        className={`text-center text-4xl font-black leading-none ${
...
function BonusStrip({
  ...
  return (
    <section className="grid gap-0 bg-[var(--nffc-black,#000000)] md:grid-cols-4">
      <div className="flex items-center justify-center bg-[var(--nffc-black,#000000)] px-4 py-3 text-center text-xl font-black uppercase tracking-[0.2em] text-[var(--nffc-white,#f5f5f5)] md:text-2xl">
        Bonuses applied this week
      </div>
      <BonusName label="Streaker" active={streakApplied} tone="yellow" />
      <BonusName label="Maverick" active={maverickApplied} tone="cyan" />
      <BonusName label="Rogue" active={rogueApplied} tone="pink" />
    </section>
  );
}
...
function TeamResultRow({
  ...
  const targetCards = Math.max(sortedPlayers.length, 5);
  const fillerCount = Math.max(0, targetCards - sortedPlayers.length);

  return (
    <div className="grid gap-px bg-[var(--nffc-white,#f5f5f5)] xl:grid-cols-[460px_1fr]">
      <div className="bg-[var(--nffc-red,#e50914)] px-3 py-2">
        <div className="flex items-start gap-3">
          <div className="px-2 py-1 text-2xl font-black leading-none text-white">
            {String(rank).padStart(2, "0")}
          </div>

          <div className="min-w-0 flex-1">
            <div className="text-2xl font-black uppercase leading-tight tracking-[0.04em] text-white md:text-3xl">
              {group.teamName}
            </div>

<div className="mt-3 flex flex-wrap items-center gap-2">
              <div
                className={`bg-[var(--nffc-black,#000000)] px-3 py-2 text-2xl font-black leading-none ${
                  correctPercentage > 0
              ...
              <div
                className={`flex items-center gap-2 bg-[var(--nffc-black,#000000)] px-3 py-2 text-2xl font-black leading-none ${
                  teamPoints > 0
                    ? "text-[var(--stat-green,#22e55e)]"
                    : "text-[var(--stat-wrong,#ff3030)]"
                }`}
      ...
      <div className="grid auto-rows-fr gap-px bg-[var(--nffc-white,#f5f5f5)] sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
        {sortedPlayers.length ? (
          <>
            ...
            {Array.from({ length: fillerCount }).map((_, index) => (
              <VoidedPlayerCard key={`${group.key}-void-${index}`} />
            ))}
          </>
        ) : (
          <div className="bg-[#1a1a1a] p-4 text-sm font-black uppercase tracking-[0.16em] text-[var(--nffc-muted,#a7a7a7)]">
            No player rows
          </div>
        )}
      </div>
...
function PlayerScoreCard({
  ...
  const nameTone = total <= 0 ? "red" : getPlayerNameTone(player, summary);

  const scoreParts = [
    { value: basePoints, tone: "green" as Tone },
    { value: streakBonus, tone: "yellow" as Tone },
    { value: maverickBonus, tone: "cyan" as Tone },
    { value: rogueBonus, tone: "pink" as Tone },
    { value: cupBonus, tone: "green" as Tone },
  ...
  return (
    <article className="grid min-h-[92px] grid-rows-[auto_1fr] bg-[var(--nffc-black,#000000)] p-2">
      <div className="flex min-w-0 items-center justify-center gap-6">
        <div className={`truncate text-center text-2xl font-black uppercase leading-tight tracking-[0.03em] md:text-3xl ${textTone(nameTone)}`}>
          {getPlayerName(player)}
        </div>

        <div className={`shrink-0 border px-3 py-1 text-lg font-black uppercase leading-none tracking-[0.08em] ${borderTextTone(correct ? "green" : "white")}`}>
          {player.prediction ?? "-"}
        </div>
      </div>

      <div className="mt-2 flex items-center justify-center gap-2 text-xl font-black uppercase leading-none md:text-2xl">
        {scoreParts.length ? (
          <>
            {scoreParts.map((part, index) => (
              <span key={`${part.tone}-${index}`} className="inline-flex items-center gap-2">
                {index > 0 && (
                  <span className="text-[var(--nffc-white,#f5f5f5)]">+</span>
                )}
                <span className={textTone(part.tone)}>
          ...
          <span className="text-[var(--stat-wrong,#ff3030)]">0</span>
        )}
...
function VoidedPlayerCard() {
  return (
    <div
      className="min-h-[92px] bg-[#1a1a1a]"
      aria-hidden="true"
    />
  );
}
