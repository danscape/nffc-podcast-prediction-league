...
function forestResultFromMatch(
...
export async function POST(request: Request) {
  try {
    ...
    if (!dryRun) {
      ...
      if (!authorised) {
        return Response.json(
          {
            success: false,
            message: "Unauthorised live create-season request.",
          },
          { status: 401 }
        );
      }
    ...
    const { data: existingData, error: existingError } = await supabase
      .from("fixtures")
      .select("id, season, gameweek, gameweek_label, external_api_id")
    ...
    for (const match of matches) {
      ...
      if (!gameweek || gameweek < 1 || gameweek > 38) {
        results.push({
          ...
          action: "skipped",
          message: "Missing or invalid matchday/gameweek.",
        });
        continue;
      }
      ...
      const existing = byExternalId.get(apiId) ?? byGameweek.get(gameweek);
      ...
      if (existing) {
        ...
        results.push({
          ...
          action: "updated",
          message: dryRun
            ? "Would update existing fixture row."
            : "Updated existing fixture row.",
        });
      ...
      results.push({
        ...
        action: "created",
        message: dryRun
          ? "Would create new fixture row."
          : "Created new fixture row.",
      });
    }
